# Agent Context

This folder holds extra project context for coding agents. Keep root
`AGENTS.md` as the short entry point, and put longer explanations here.

Suggested reading:

- `architecture.md`: how the package is layered.
- `workflows.md`: command behavior and user-facing flows.
- `testing.md`: local and opt-in Modal validation.
- `../custom-harness.md`: executor, audit, and fake testing workflow.
- `../harness-integration.md`: how external harnesses should use the SDK.
- `../registry.md`: dynamic tool discovery and argument schemas.
- `../protocol-adapters.md`: how to expose registry tools through protocols.
- `../openai-agents-sdk-plan.md`: implementation plan for OpenAI Agents SDK support.
- `../security.md`: safety responsibilities and tool boundaries.
